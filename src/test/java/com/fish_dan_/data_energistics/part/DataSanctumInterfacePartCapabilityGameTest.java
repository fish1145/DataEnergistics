...
public final class DataSanctumInterfacePartCapabilityGameTest {

    private static final BlockPos PART_HOST_POS = new BlockPos(2, 1, 2);

    private DataSanctumInterfacePartCapabilityGameTest() {}

    @TestHolder("data_sanctum_interface_part_exposes_return_inventory_capability")
    @EmptyTemplate("5")
    ...
    public static void exposesReturnInventoryCapability(GameTestHelper helper) {
        ...
        IPart installedPart = cableBus.addPart(ModItems.DATA_SANCTUM_INTERFACE_PART.get(), Direction.NORTH, null);
        if (!(installedPart instanceof DataSanctumInterfacePart interfacePart)) {
            throw new GameTestAssertException("The cable bus must install a data sanctum interface part");
        }
        ...
        if (returnInventory == null) {
            throw new GameTestAssertException("The data sanctum interface part must expose its return inventory");
        }

        helper.assertTrue(returnInventory == interfacePart.getReturnInventory(),
                "The capability must return the interface part's own return inventory");
        ...
        helper.assertValueEqual(inserted, 1L, "The capability must write into the part return inventory");
        helper.assertValueEqual(interfacePart.getReturnInventory().getAmount(0), 1L,
                "The part return inventory must observe the capability write");

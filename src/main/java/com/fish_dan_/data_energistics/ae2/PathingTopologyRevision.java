 ...
 *
 * <p>The public AE2 pathing API exposes the current state but not a mutation generation. The PathingService Mixin
 * implements this bridge so grid-local scalar caches can avoid rescanning controller geometry on every server tick
 * while still invalidating immediately after a repath.</p>
 */

 ...
 * @param stored      FE stored when the snapshot was captured, saturated to one {@code long} for virtual buffers
 * @param capacity    maximum FE the endpoint can store, saturated to one {@code long} for virtual buffers
...
public record TowerEnergyEndpointSnapshot(TowerEnergyEndpointId endpoint,
    ...
    public TowerEnergyEndpointSnapshot {
        ...
        if (extractable < 0) {
            throw new IllegalArgumentException("Extractable energy must not be negative");
        }
        if (receivable < 0) {
            throw new IllegalArgumentException("Receivable energy must not be negative");
        }
        if (stored > capacity) {
            throw new IllegalArgumentException("Stored energy must not exceed capacity");
        }
        if (role == TowerEnergyEndpointRole.BALANCED) {
            if (extractable > stored) {
                throw new IllegalArgumentException("Extractable energy must remain within stored energy");
            }
            if (receivable > capacity - stored) {
    ...
    /**
     * Creates a virtual buffer whose extraction and insertion budgets are independent limits for one transaction.
     *
     * <p>
     * Network storage APIs expose each operation through a {@code long}; neither returned budget describes the
     * backing store's absolute contents or capacity. The legacy scalar state remains a saturated display projection,
     * while planning consumes the two explicit budget fields independently.
     * </p>
     *
     * @param endpoint    stable endpoint identity
     * @param extractable maximum FE the buffer can provide in this transaction
     * @param receivable  maximum FE the buffer can accept in this transaction
     * @return immutable virtual-buffer snapshot
     */
    public static TowerEnergyEndpointSnapshot buffer(
        ...
        return new TowerEnergyEndpointSnapshot(
                endpoint,
                extractable,
                saturatingAdd(extractable, receivable),
                extractable,
                receivable,

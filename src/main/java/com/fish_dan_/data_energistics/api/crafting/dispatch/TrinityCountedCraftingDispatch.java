 ...
 *
 * <p>Registration and unregistration are provider lifecycle operations and must run on the logical server thread.
 * Adapters are keyed by provider object identity, not {@link Object#equals(Object)}. A provider may have only one
 * registered adapter at a time.</p>
 *
 * <p>Third-party integrations should compile only against the DataEnergistics API and declare DataEnergistics as an
 ...
 * otherwise reference these API types; this prevents JVM class linking to DataEnergistics when the mod is absent.</p>
 */
public final class TrinityCountedCraftingDispatch {
     ...
     *
     * <p>An adapter cannot replace DataEnergistics' direct counted-provider contract. Duplicate registration of the

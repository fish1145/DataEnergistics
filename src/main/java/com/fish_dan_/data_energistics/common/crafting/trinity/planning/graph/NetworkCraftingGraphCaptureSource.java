...
public final class NetworkCraftingGraphCaptureSource implements TrinityCraftingGraphCaptureSource {
    ...
    NetworkCraftingGraphCaptureSource(NetworkCraftingProviders providers,
        ...
        this.providers = Objects.requireNonNull(providers, "A Trinity graph source requires AE2 providers");
        this.registries = Objects.requireNonNull(registries, "A Trinity graph source requires registry access");
        this.revision = Objects.requireNonNull(revision, "A Trinity graph source requires a revision");
    }
    ...
    public List<IPatternDetails> capturePatternsFor(AEKey primaryOutput) {
        Objects.requireNonNull(primaryOutput, "A Trinity graph source lookup requires a primary output");
    ...
    private static LongSupplier requiredRevisionSource(NetworkCraftingProviders providers) {
        Objects.requireNonNull(providers, "A Trinity graph source requires AE2 providers");
        if (providers instanceof TrinityCraftingProviderRevision revision) {
            return revision::trinityCraftingProviderRevision;
        }

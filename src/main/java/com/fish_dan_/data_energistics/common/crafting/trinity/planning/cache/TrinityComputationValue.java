 ...
 * @param registered whether the calculation occupies a Grid LRU entry
 * @param <V>        result type
 */
public record TrinityComputationValue<V>(V value, boolean cacheHit, boolean registered) {
    ...
    public TrinityComputationValue {
        ...
        if (cacheHit && !registered) {
            throw new IllegalArgumentException("A Trinity inline cache hit must refer to a registered entry");
        }
    }
}

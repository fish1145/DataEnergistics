...
public final class TrinityCraftingGraphRebuilder {
    ...
    public record AdvanceResult(AdvanceStatus status, long targetRevision, int capturedPatternCount) {
        ...
        public AdvanceResult {
            Objects.requireNonNull(status, "A Trinity graph advance requires a status");
    ...
    public TrinityCraftingGraphRebuilder(TrinityCraftingGraphCaptureSource source, LongSupplier nanoTime) {
        this.source = Objects.requireNonNull(source, "A Trinity graph rebuilder requires a source");
        this.nanoTime = Objects.requireNonNull(nanoTime, "A Trinity graph rebuilder requires a clock");
    }
    ...
    private AdvanceResult advanceCapture(long budgetNanos) {
        ...
        while (true) {
            BuildState active = Objects.requireNonNull(this.build);
            ...
            if (details == null) {
                throw new IllegalArgumentException("A Trinity graph source cannot publish a null pattern");
            }
    ...
    private boolean beginBuild(long revision) {
        HolderLookup.Provider registries = Objects.requireNonNull(this.source.registries(), "A Trinity graph source requires registry access");
        List<AEKey> craftableKeys = List.copyOf(this.source.captureCraftableKeys());
        for (AEKey key : craftableKeys) {
            Objects.requireNonNull(key, "A Trinity graph source cannot publish a null craftable key");
        }
    ...
    private AdvanceResult progress() {
        BuildState active = Objects.requireNonNull(this.build);

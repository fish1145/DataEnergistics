...
public record TrinityCycleSelection(
    ...
    /**
     * Validates top-level accounting while trusting the planner-owned collection elements.
     */
    public TrinityCycleSelection {
        ...
        minimumSeed = amountsView(minimumSeed, false);
        initialInputs = amountsView(initialInputs, false);
        netChange = amountsView(netChange, true);
        exportableNet = amountsView(exportableNet, false);
        retainedSeed = amountsView(retainedSeed, false);
    ...
    private static Map<AEKey, BigInteger> amountsView(Map<AEKey, BigInteger> source, boolean signed) {
        if (source.values().stream().anyMatch(amount -> signed ? amount.signum() == 0 : amount.signum() <= 0)) {

...
public class DataFlowPortableCellItem extends PortableCellItem {
    ...
    private static ItemStack getStorageComponent(ItemStack stack) {
        ...
        if (item == ModItems.PORTABLE_DATA_FLOW_CELL_1M.get()) {
            return ModItems.DATA_STORAGE_COMPONENT_1M.toStack();
        }
        if (item == ModItems.PORTABLE_DATA_FLOW_CELL_4M.get()) {
            return ModItems.DATA_STORAGE_COMPONENT_4M.toStack();
        }
        if (item == ModItems.PORTABLE_DATA_FLOW_CELL_16M.get()) {
            return ModItems.DATA_STORAGE_COMPONENT_16M.toStack();
        }
        if (item == ModItems.PORTABLE_DATA_FLOW_CELL_64M.get()) {
            return ModItems.DATA_STORAGE_COMPONENT_64M.toStack();
        }
        if (item == ModItems.PORTABLE_DATA_FLOW_CELL_256M.get()) {

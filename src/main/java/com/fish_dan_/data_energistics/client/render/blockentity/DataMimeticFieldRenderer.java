...
public class DataMimeticFieldRenderer implements BlockEntityRenderer<DataMimeticFieldBlockEntity> {
    ...
    // BlockEntityRenderDispatcher recreates renderer instances after resource reloads.
    private @Nullable CarrierModels carrierModels;
    ...
    public void render(DataMimeticFieldBlockEntity blockEntity, float partialTick, PoseStack poseStack,
        ...
        CarrierModels models = getCarrierModels();
        ...
        for (int slot = 0; slot < visibleSlots; slot++) {
            ...
            BakedModel model = models.forStack(stack);
            if (model == null) {
                continue;
            }
    ...
    private CarrierModels getCarrierModels() {
        CarrierModels current = carrierModels;
        if (current == null) {
            current = new CarrierModels(
                    modelManager.getModel(MOB_CARRIER_MODEL),
                    modelManager.getModel(ORE_CARRIER_MODEL),
                    modelManager.getModel(CROP_CARRIER_MODEL));
            carrierModels = current;
        }
        return current;
    }
    ...
    private record CarrierModels(BakedModel mob, BakedModel ore, BakedModel crop) {

        private @Nullable BakedModel forStack(ItemStack stack) {

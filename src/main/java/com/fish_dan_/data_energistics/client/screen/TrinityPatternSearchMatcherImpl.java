...
/**
 * Implements locale-independent search text construction for Trinity pattern candidates.
 */
public final class TrinityPatternSearchMatcherImpl implements TrinityPatternSearchMatcher {
    ...
    public String createSearchText(List<String> inputs, List<String> outputs, TrinityPatternSearchMode mode) {
        ...
        List<String> normalizedInputs = normalize(inputs, "inputs");
        List<String> normalizedOutputs = normalize(outputs, "outputs");
    ...
    public boolean matchesSearchText(String searchText, String query) {
        Objects.requireNonNull(searchText, "searchText must not be null");
     ...
     *
     * @param candidates     names to validate and normalize
     * @param collectionName collection label used in fail-fast diagnostics
     ...
     */
    private static List<String> normalize(List<String> candidates, String collectionName) {
        Objects.requireNonNull(candidates, collectionName + " must not be null");
        ArrayList<String> normalizedCandidates = new ArrayList<>(candidates.size());
        for (String candidate : candidates) {
            normalizedCandidates.add(Objects.requireNonNull(
                    candidate,
                    collectionName + " must not contain null elements").toLowerCase(Locale.ROOT));
        }

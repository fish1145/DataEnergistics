...
public final class TimeShiftRecipeSerializer implements RecipeSerializer<TimeShiftRecipe> {
    ...
    private static final Codec<List<TimeShiftIngredient>> INGREDIENTS_CODEC = TimeShiftIngredient.CODEC.codec().listOf()
            .flatXmap(
                    ingredients -> ingredients.isEmpty() ? DataResult.error(() -> "Time shift recipe must have at least one ingredient") : DataResult.success(ingredients),
                    DataResult::success);
    private static final Codec<List<ItemStack>> RESULTS_CODEC = ItemStack.CODEC.listOf()
            .flatXmap(
                    results -> results.isEmpty() ? DataResult.error(() -> "Time shift recipe must have at least one result") : DataResult.success(results),
                    DataResult::success);
    private static final Codec<Double> MINUTES_CODEC = Codec.DOUBLE
            .flatXmap(
                    minutes -> minutes <= 0.0D ? DataResult.error(() -> "Time shift minutes must be greater than 0") : DataResult.success(minutes),
                    DataResult::success);
    ...
    private static final MapCodec<TimeShiftRecipe> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            ...
            TimeShiftTimeCondition.CODEC.optionalFieldOf("time", TimeShiftTimeCondition.ALL).forGetter(TimeShiftRecipe::getTimeCondition)).apply(instance, TimeShiftRecipe::new));

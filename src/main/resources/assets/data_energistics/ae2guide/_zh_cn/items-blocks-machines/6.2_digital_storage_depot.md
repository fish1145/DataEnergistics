...
>数位化存储仓是一种可接入 ME 网络的混合存储方块。它可以同时保存物品、流体，以及数据流、二进制等非物品包裹堆栈。
...
- 包裹堆栈槽每格基础容量为 64,000，适合保存数据流、二进制等非物品、非流体包裹堆栈
...
AE2 导出总线也可以配合存储仓使用。把数据流、二进制等包裹堆栈配置到导出总线后，可以直接导入存储仓的包裹堆栈槽；对普通物品和流体则按对应槽位处理。

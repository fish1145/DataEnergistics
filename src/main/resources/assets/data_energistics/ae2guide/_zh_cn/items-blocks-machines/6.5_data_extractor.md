...
>
逻辑有点复杂，建议参考一下下面说的话.  
>⚠ 主要用途就是通过白值高的剑类武器攻击目标产出数据流，以及使用数据载体记录对应物品或生物，制作对应的数据载体  
>(大部分无限伤害的武器基本都是依靠代码的，所以在这里会显示成它最原本应该的伤害,包括但不限于寰宇支配之剑之类的)
...
其中：
- `carrierRules` 控制提取仪各槽位允许放入什么、记录成什么数据、每个物品提供多少进度、以及总需求量
- `outputRules` 控制成品数据载体在数据拟生场中会产出什么
- 各数组中相同索引的值组成一行规则，YAML 注释说明每一列的含义

新建规则文件时会写入内置默认行；已有数组会直接按配置使用，清空后不会读取旧配置或额外恢复另一套默认表。
如果没有对应的 `outputRules` 条目，数据拟生场会退回到默认战利品表或硬编码掉落逻辑。

...
- 旧 TOML 导入器识别 `ae910ee2` 的 64 个基线输入，并额外兼容历史 `mipTimeoutMs`；目标 YAML 恰好 63 个叶字段。
- 领域计数为通用 9、Data Extractor 14、Flattening TNT / Data Nuke 14、Solar 4、Trinity Crafting 8、Trinity Dispatch 14。
- Data Extractor 规则使用独立的原生 `data_extractor_rules.yaml`，并注册为支持严格实时重载的第二个 Configuration Holder；旧 JSON 仅作首次导入源。
- 所有实现类型按职责命名，禁止使用统一实现后缀命名约定。

## 核心原则

1. 已存在且有效的 YAML 永远优先；旧 TOML 不覆盖、不删除。
2. `displayName` 丢弃，基线之外的历史 `mipTimeoutMs` 忽略；`safeRetryBackoffTicks` 显式值迁移，旧文件缺失时补 8。
3. TOML `maxBindingVariants=512` 升级为 32768；YAML 显式 512 保留。
4. 启动注册 Configuration 前严格预校验 YAML，防止框架自动纠正坏文件。
5. 主 YAML 与规则 YAML 使用 Configuration 内置 `FileWatchManager`，不建立项目 watcher；服务器 tick 桥接分别原子发布完整主配置和完整规则快照。
...
- [06-data-extractor-rules.md](06-data-extractor-rules.md)：活动规则 YAML、旧 JSON 导入和严格实时发布。

...
| 多方块自动建造 | `MultiBlockAutoBuildOverlay` | LDLib2 弹窗或抽屉式子视图 |
...
采用“纯预览模型 + 共用 LDLib2 UI + XEI 薄适配器 + 服务端动作”的架构。方块 UI 与 XEI 页面共享展示组件，但 XEI 页面不得直接改世界、库存或样板。实施已在 `qy/ldlib2-ui-migration` 分支启动；每个工作包按功能独立提交，未通过对应验收前保留旧入口。

...
当前状态必须作为续做起点：DataEnergistics 位于 qy/ldlib2-ui-migration，Draft PR https://github.com/fish1145/DataEnergistics/pull/99，除既有设计/依赖提交外，已按功能推送 2df0ed28（atomic definition snapshot）、5cdc4ae7（MDLib paired jar）、89a81c7b（DataE predicate pair 适配）、a74fe20a（common preview 模型）、68a2f3e8（Trinity preview catalog）、dd41ae4e（Lombok getter 与共享 key）。MDLib 位于 qy/multiblock-preview-api，Draft PR https://github.com/ModularMCLib/Modular-Data-lib/pull/2，已在原有 projector/candidate 提交之上推送 cbbbbd7（PatternCandidate 与 patternCandidates）。MDLib 已通过 spotlessCheck/test/runGameTestServer/build，GameTest 4/4；DataEnergistics 已通过 IDEA 编译、spotlessCheck/test/runGameTestServer/build，GameTest 290/290。不要重复这些工作，也不要再次等待推送授权。
...
common preview spec/selection/snapshot/material/revision 与 Trinity catalog 已完成。从当前状态继续按以下顺序推进：AE menu bridge、AeItemSlot、HostUiExtension；Trinity Data Core 垂直切片；Pattern Core 与自动建造浮窗；ME_OUTPUT、ME_INPUT、组合仓库、PATTERN_BUFFER；JEI/EMI 共用预览；typed AE transfer；旧入口清理。测试必须直接调用目标逻辑，禁止源码 contain 测试、删除功能测试和反射测试。完成每个工作包后更新文档状态，执行对应逻辑/容器测试；最终在 JEI、EMI、单人和多人环境验证 UI、材料、槽位、自动建造、资源重载、恶意请求、样板原子性与渲染资源释放。只有所有验收通过且无未处理阻塞项才可结束 Goal。
```

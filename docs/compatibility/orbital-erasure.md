...
命中使用完整实体碰撞箱与实际攻击体积的交集，不以玩家脚底或实体中心单点代替身体。

- 定向能采用与渲染同宽、同朝向的有限方形光束体积，检查炮口到已处理前沿的整段光束，包括构建高度以上的部分。末端是平整截面，不因宽度增加而伤及前沿之后的目标；区块或预算等待期间，已经显示的光束仍保持碰撞。每个实际处理的射线段查询一次实体，地形游标和逐格方块预算不变。
- 定向能所选半径约束扫描中心路径；实体命中还包括光束自身截面与目标身体的大小，不能把刚刚离开中心路径圆盘的位置当成安全边界。
- 动能覆盖冲击球、完整贯穿柱和实际陨石坑体积。贯穿柱包含其完整方块单元，坑体沿用当前攻击保存的碗状或旧圆柱形状，保留的侧壁不会被当成已挖除空间。
- 轨道数位湮灭体按当前激活球体与碰撞箱求交，身体部分进入球体即命中；实体判定不再被构建高度截断。手动点燃的非轨道数位湮灭体保留原有规则。
...
- 本次结算新生成的龙心记录为本次打击已保护的主体，避免后续光束或球体立即抹除自己的奖励；保护仅属于产生奖励的这次打击，不形成永久物品免疫。

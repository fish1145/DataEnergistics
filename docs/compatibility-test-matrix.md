...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ae2lt` | Yes | Yes | `compileOnly` + `runtimeOnly` | `Ae2LtCompat.isLoaded()`, `OptionalMods.isLoaded()` | `Ae2LtMixinGuards.isPresent()` plus AE2LT sentinel resource in `DataEnergisticsMixinPlugin` | `Ae2LtRuntimeBridge`, `Ae2LtWirelessBridge` | Not verified | **Client startup flow entered** in baseline dev runtime; dedicated server failed before completion | **High** | Highest-risk optional surface due to guarded mixins and internal reflection; keep dedicated absent/present runs separate. |
| `ae2wtlib` | Yes | Yes | `compileOnly` + `runtimeOnly` (+ extracted API jar as `compileOnly`) | `Ae2WtLibCompat` load check via `OptionalMods.isLoaded()` | No dedicated mixin guard | `Ae2WtLibCompat` screen replacement reflection | Not verified | **Client startup flow entered** in baseline dev runtime | Medium | Needs dedicated run without AE2WTLib and a client path that opens the relevant terminal screen. |
